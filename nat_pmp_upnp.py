...
def add_port_mapping():
    ...
    try:
        ...
        if confirm_port():
            babase.screenmessage("You are now joinable from the internet", (0.2, 1, 0.2))
...
def check_port():
    ...
    if devices == []:
        babase.screenmessage("Please enable upnp service on your router", (1.00, 0.15, 0.15))
        ...
        for service in upnp_dev.services:
            if service in WAN_SERVICE_NAMES:
                ...
                if service.GetSpecificPortMappingEntry:
                    try:
                        result = service.GetSpecificPortMappingEntry(NewRemoteHost='', NewExternalPort=43210, NewProtocol="UDP")
                        if result and confirm_port():
                            return True
                        elif result and not confirm_port():
                            if babase.do_once():
                                babase.screenmessage("Oops seems like your network doesn't support upnp", (1.0, 0.15, 0.15))
                                babase.pushcall(bui.getsound('error').play(), from_other_thread=True)
                            return True
